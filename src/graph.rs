...
use crate::vertex::{Vertex, Edges, Direction};
use crate::edge::Edge;
    ...
    Ix: Index + Debug
{
    ...
    pub fn add_edge(&mut self, edge: (&Vertex<T, Ix>, &Vertex<T, Ix>)) {
        ...
        if let Ok(_) = self.check_cycles(edge.clone().into()) {
    ...
    ///```
    pub fn extend_from_edges(
        &mut self, 
        edges: Vec<(&Vertex<T, Ix>, &Vertex<T, Ix>)>
    ) {
        edges.iter().for_each(|e| {
            let mut source = e.0.get_index();
            let mut reference = e.1.get_index();
            let edge: Edge<Ix> = e.to_owned().into();
            ...
            if let Some(r) = self.get_vertex(reference) {
                ...
                let edge = e.to_owned().into();
                let mut r = e.1.clone();
                r.add_edge(&edge);
            }
            self.add_edge(e.to_owned());
        });
    }
    ...
    fn add_vertices(&mut self, vertices: Vec<&Vertex<T, Ix>>) {
    ...
    fn auto_source_cycle(&self) -> bool {
        self.n_roots() == 0 && self.len() != 0
    }

    fn auto_ref_cycle(&self) -> bool {
        self.n_leaves() == 0 && self.len() != 0
    }
    ...
    pub(crate) fn topological_sort(&self) -> GraphResult<Ix> {
        ...
        for root in roots {
            if let Some(vtx) = self.get_vertex(root.clone()) {
                if let Err(e) = self.dfs(vtx, &mut stack, &mut visited) {
                    return Err(e)
                };
            }
        }
        ...
        if !references.is_empty() {
            for r in references {
                if let Some(vtx) = self.get_vertex(r.clone()) {
                    if let Err(e) = self.dfs(vtx, stack, visited) {
                        return Err(e)
                    }
                }
            }
